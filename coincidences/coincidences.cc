...
int main(int argc,char *argv[]) {
  ...
  //Objetos de root donde se guarda todo lo calculado
  TFile *DataFile = new TFile( argv[1] );
  TTree *dataTree = 0;
  TH1D *aHist = new TH1D("h1","Tiempos de decaimiento detectados",40,0,20000);
  ...
  // Nuevas flag que nos serviran para saber si hubo decaimiento
  bool coincidenceAnC = false;
  bool coincidenceAnBnC = false;
  ...
  for(int i=0; i<n_events; i++)
  {
    ...
    coincidence = false;
    coincidenceAnC = false;
    coincidenceAnBnC = false;
    ...
    /*Chequeamos si hubo coincidencia en A y C pero no En ABC*/
    if(coincidenceAnC && !coincidenceAnBnC)
    {
      
      
      if((RE0->size() > 0) && (RE1->size() > 0)) // 0 y 1m
      {
      /*Filtramos ruido que venga de apartir de cosas que no se consideran coincidencias pero sugien muy cerca*/
       if( fabs( RE0->at(0)-RE1->at(0) ) >= 500)
       {
       aHist->Fill( fabs( RE0->at(0)-RE1->at(0) ) );
       std::cout << "found decay at " << i << std::endl;
       }    
      }
    }
  ...
  DataFile = new TFile( "datosenc7.root","RECREATE");
